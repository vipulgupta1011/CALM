...
parser.add_argument('--model', type=str, default='bigscience/T0pp')
...
model = AutoModelForSeq2SeqLM.from_pretrained(NAME_MODEL, device_map='auto', torch_dtype=torch.bfloat16)
...
def save_result(dataset, out_file, batch_size) :  
    ...
    for idx_batch in tqdm(idx_batches):
        ...
        generated_texts = tokenizer.batch_decode(generated_tokens_org, skip_special_tokens=True)
