...
file_path = os.path.join(os.path.dirname(__file__), '../../../', 'dataset/snli/test.csv')
...
for i in range(1, len(dataset)) :
    row = dataset[i]
    sentence1, sentence2, coding = row[0], row[1], row[2]
    if coding == '0' :
        answer = 'entailement'
    if coding == '1' :
        answer = 'neutral'
    ...
    if check_word_in_dictionary(sentence1.lower(), names_dict) and check_word_in_dictionary(sentence2.lower(), names_dict) :
        ...
        template['source_dataset'] = "SNLI"

        #if random.randint(0,1) == 0 :
        k +=1
        templates[k] = template

save_json('snli_templates_basic.json', templates)
print (j)
print (k)
